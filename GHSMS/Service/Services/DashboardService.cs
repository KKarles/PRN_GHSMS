...
namespace Service.Services
{
    public class DashboardService : IDashboardService
    {
        ...
        private async Task<RevenueStatsDto> GetRevenueStatsInternalAsync()
        {
            ...
            var monthlyTrends = new List<MonthlyRevenueDto>();
            for (int i = 11; i >= 0; i--)
            {
                var month = now.AddMonths(-i);
                var monthStartDate = new DateTime(month.Year, month.Month, 1);
                var monthEndDate = monthStartDate.AddMonths(1).AddDays(-1);
                var monthBookings = await _testBookingRepo.GetBookingsByDateRangeAsync(monthStartDate, monthEndDate);
                
                monthlyTrends.Add(new MonthlyRevenueDto
                {
                    Month = month.ToString("yyyy-MM"),
                    Revenue = monthBookings.Where(b => b.IsPaid).Sum(b => b.Service?.Price ?? 0),
                    BookingCount = monthBookings.Count(b => b.IsPaid)
                });
            }

            // Revenue by service
            var paidBookings = await _testBookingRepo.GetPaidBookingsAsync();
            var revenueByService = paidBookings
                .GroupBy(b => new { b.ServiceId, ServiceName = b.Service?.ServiceName ?? "Unknown" })
                .Select(g => new ServiceRevenueDto
                {
                    ServiceName = g.Key.ServiceName,
        ...
        private async Task<UserStatsDto> GetUserStatsInternalAsync()
        {
            ...
            var registrationTrends = new List<DailyRegistrationDto>();
            for (int i = 29; i >= 0; i--)
            {
                var date = now.AddDays(-i).Date;
                var count = allUsers.Count(u => u.CreatedAt?.Date == date);
                registrationTrends.Add(new DailyRegistrationDto
                {
                    Date = date,
            ...
            return new UserStatsDto
            {
                ...
                ActiveUsers = allUsers.Count(u => u.UpdatedAt >= now.AddDays(-30)),
